from flask import Flask, render_template, request, redirect, url_for, send_file, session, flash, jsonify, Response, Blueprint
...
def ask_ollama(prompt: str) -> str:
    """
    Sends a prompt to the Ollama CLI using the 'run' command or queries the database for data analysis.

    Args:
        prompt (str): The user's input prompt.

    Returns:
        str: The formatted response generated by the Ollama CLI or database query.
    """
    try:
        # Check if the prompt is a data analysis request
        if "data analysis" in prompt.lower() or "total cases" in prompt.lower() or "types of cases" in prompt.lower():
            return handle_data_analysis(prompt)

        # Use the CLI to run the model and generate a response
        ...
        if result.returncode == 0:
            ...
            return f"Error communicating with Ollama CLI: {result.stderr.strip()}"
    except subprocess.TimeoutExpired:
        return "Error: The request to the Ollama CLI timed out. Please try again with a shorter prompt or later."
    except Exception as e:
        return f"Error executing Ollama CLI: {str(e)}"

def handle_data_analysis(prompt: str) -> str:
    """
    Handles data analysis requests by querying the forensic.db database.

    Args:
        prompt (str): The user's input prompt.

    Returns:
        str: A formatted response with the requested data analysis.
    """
    try:
        # Log the incoming prompt
        print(f"handle_data_analysis called with prompt: {prompt}")

        # Query the database for data analysis
        with app.app_context():
            total_cases = Case.query.count()
            completed_cases = Case.query.filter_by(status='completed').count()
            pending_cases = Case.query.filter_by(status='pending').count()
            assigned_cases = Case.query.filter_by(status='assigned').count()
            case_types = db.session.query(Case.analysis_type, func.count(Case.analysis_type)).group_by(Case.analysis_type).all()

        # Build the response
        response = f"Here is the data analysis:\n\n"
        response += f"Total Cases: {total_cases}\n"
        response += f"Completed Cases: {completed_cases}\n"
        response += f"Pending Cases: {pending_cases}\n"
        response += f"Assigned Cases: {assigned_cases}\n\n"
        response += "Case Types:\n"
        ...
        # Log the error
        print(f"Error fetching data analysis: {str(e)}")
        return f"Error fetching data analysis: {str(e)}"

def format_response(raw_response: str) -> str:
    """
    Formats the raw response into a professional, readable style with line breaks and bullet points.

    Args:
        raw_response (str): The raw response from the Ollama CLI.
    ...
    for line in lines:
        line = line.strip()
        if line:
            # Add bullet points for numbered or list-like content
...
def chat():
    ...
    if not user_id or not message:
        return jsonify({"reply": "Missing user_id or message"}), 400
    ...
    # Build prompt
    prompt = f"[System Prompt] {get_system_prompt()}\n"
    for role, msg in history:
        prompt += f"[{role.capitalize()}] {msg}\n"
    prompt += "[Agent] "

    # Use the ask_ollama function to generate a response
    response = ask_ollama(prompt)

    # Save agent response
    add_to_history(user_id, "agent", response)
...
def add_to_history(user_id, role, message):
    ...
    user_sessions[user_id].append((role, message))
    user_sessions[user_id] = user_sessions[user_id][-10:]  # Keep last 10 messages

def get_system_prompt():
    return (
        "You are a real-time forensic investigation AI assistant. "
        "You analyze cybercrime, trace digital trails, and help understand suspicious incidents. "
        "Be clear, investigative, and highly logical in your replies."
    )
